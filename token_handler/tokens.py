    ...
    try:
        ...
        if tokens:
            ...
            if expiration_time > int(time.time()):  # If not expired
                ...
                redis_client.expire(f"remodel_id:{remodel_id}#", 3600) 
        ...
        if expiration_time < int(time.time()):#check if current time bigger than expiration
            new_tokens = refresh_token(remodel_id, refresh_token_value)
            ...
            redis_client.expire(f"remodel_id:{remodel_id}#", 3600)
            update_zoho_creds(remodel_id, tokens)#update the new tokens in database
